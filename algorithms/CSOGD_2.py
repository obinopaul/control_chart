...
def CSOGD_2(y_t, x_t, model, eta_p, eta_n, ratio_Tp_Tn, method='sum'):
    """
    ...
    # Calculate the loss using Hinge Loss II
    loss = (rho if y_t == 1 else 1) * max(0, 1 - y_t * np.dot(w, x_t))
