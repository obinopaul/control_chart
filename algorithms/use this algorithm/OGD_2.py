...
def OGD_1(y_t, x_t, model, eta_p, eta_n, ratio_Tp_Tn):
    ...
    # Prediction
    f_t = np.dot(w, x_t)  # Use dot product for 1D arrays
    ...
    # Cost-sensitive Hinge Loss
        ...
        l_t = (rho if y_t == 1 else 1) * max(0, 1 - y_t * f_t)  # Cost-sensitive Hinge Loss II
        if l_t > 0:
            w += eta_t * y_t * x_t  # Update w with hinge loss derivative
