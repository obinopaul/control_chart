...
def CSRDA_2(y_t, x_t, model, eta_p, eta_n, num_positive, num_negative):
    """
    CSRDA-II: Cost-sensitive Regularized Dual Averaging (Type II) learning algorithm
    ...
    Algorithm Details:
    - Loss Function (Eq. 2): ℓ_II(w; (x,y)) = ᾱ · max(0, 1 - yw^T x)
    - Gradient Update (Eq. 8): ḡ_t = (t-1)/t · ḡ_{t-1} - ᾱ_t·κ_t·y_t/t · x_t (if loss > 0)
    ...
    Note: CSRDA-II generally performs better than CSRDA-I (see Table IV in paper)
    """
    ...
    # Algorithm 1, Line 5: κ_t = κ * (ratio)
    ...
    # Cost-Sensitive Hinge Loss Type II (Equation 2)
    # ℓ_II(w; (x,y)) = ᾱ · max(0, 1 - yw^T x)
    hinge_loss = max(0, 1 - y_t * f_t)
    l_t = float(alpha_t * hinge_loss)
    
    # Gradient Update (Equations 8 and 6)
    if l_t > 0:
        # Algorithm 1, Line 20: update ḡ_t = (t-1)/t · ḡ_{t-1} - ᾱ_t·κ_t·y_t/t · x_t
        # Note: Different from CSRDA-I by the additional κ_t term
        g_bar = ((model.t - 1) / model.t) * g_bar - (alpha_t * y_t / model.t) * x_t
    else:
        # Algorithm 1, Line 23: update ḡ_t = (t-1)/t · ḡ_{t-1}
