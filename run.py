...
def extract_window_length_from_dataset(dataset_name):
    """
    Extract the window length (w) from the dataset filename.
    
    Expected format: abtype1_w50_t0.5.libsvm or similar
    Returns the window length as an integer, or None if not found.
    
    Args:
        dataset_name: Path to dataset file (e.g., 'data/abtype1/abtype1_w50_t0.5.libsvm')
    
    Returns:
        Window length as integer, or None if pattern not found
    """
    import re
    ...
    # Look for pattern like "w10", "w20", "w45", "w100", etc.
    match = re.search(r'_w(\d+)', base_name)
...
def run(task_type, algorithm_name, dataset_name, file_format, nb_runs=1, shuffle_data=True, print_results=True, test_parameters=False, loss_type=None):
    ...
    # # For CSRDA algorithms, dynamically set L range based on dataset window length
    # if algorithm_name.upper() in ['CSRDA_1', 'CSRDA_2']:
    #     dataset_window = extract_window_length_from_dataset(dataset_name)
    #     if dataset_window is not None:
    #         _options.set_csrda_L_range(dataset_window)
    #         print(f"CSRDA: Detected dataset window length w={dataset_window}, setting L range to {_options.range_L}")
    #     else:
    #         print(f"CSRDA: Could not detect window length from dataset name, using default L range: {_options.range_L}")
    ...
    for i in range(nb_runs):
        ...
        save_individual_run_results(result, algorithm_name, dataset_name, i + 1)
...
def save_hyperparameters(algorithm_name, best_hyperparameters, dataset_name):
    ...
    # main_path = os.path.join("/ourdisk/hpc/disc/obinopaul/auto_archive_notyet/tape_2copies/CCPR/best_hyperparameters", middle_part)
...
def save_individual_run_results(result, algorithm_name, dataset_name, run_number):
    """
    Saves the results of a single run to a CSV file.
    Structure: results/individual_runs/{abtype}/{dataset_name}/{algorithm_name}_run_{run_number}.csv
    """
    # Extract base name without extension (e.g., abtype1_w50_t0.5)
    base_name = os.path.splitext(os.path.basename(dataset_name))[0]
    
    # Extract abtype folder (e.g., abtype1)
    # Assuming dataset_name is like 'data/abtype1/filename.libsvm'
    # We want to extract 'abtype1'
    try:
        # Normalize path separators
        norm_path = os.path.normpath(dataset_name)
        parts = norm_path.split(os.sep)
        
        # Find 'data' and get the next part, or just take the parent folder of the file
        if 'data' in parts:
            data_idx = parts.index('data')
            if data_idx + 1 < len(parts):
                abtype_folder = parts[data_idx + 1]
            else:
                abtype_folder = "unknown_abtype"
        else:
            # Fallback: use the parent folder name
            abtype_folder = os.path.basename(os.path.dirname(dataset_name))
            if not abtype_folder:
                abtype_folder = "unknown_abtype"
    except Exception:
        abtype_folder = "unknown_abtype"

    # Construct the output directory path
    # results/individual_runs/{abtype}/{dataset_base_name}/
    output_dir = os.path.join("results", "individual_runs", abtype_folder, base_name)
    
    # Create directory if it doesn't exist
    os.makedirs(output_dir, exist_ok=True)
    
    # Construct the filename
    filename = f"{algorithm_name}_run_{run_number}.csv"
    file_path = os.path.join(output_dir, filename)
    
    # Prepare data for DataFrame
    data = {
        'Captured Time': result['captured_t'],
        'Mistakes': result['mistakes'],
        'Updates': result['nb_SV'],
        'Time': result['ticks'],
        'Accuracies': result['accuracies'],
        'Sensitivities': result['sensitivities'],
        'Specificities': result['specificities'],
        'Precisions': result['precisions'],
        'G-Mean': result['gmeans'],
        'Kappas': result['kappas'],
        'MCCs': result['mccs'],
        'Cumulative Errors': result['cumulative_errors']
    }
