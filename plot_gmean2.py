...
def plot_gmean_vs_window_length(abtype, t_value):
    """Plot G-Mean vs Window Length for all algorithms in one plot."""
    ...
    for algorithm in algorithms_to_plot:
        ...
        for w in window_lengths_to_plot:
            ...
            if os.path.exists(file_path):
                ...
                # Check if G-Mean is available
                if not df_algorithm.empty:
                    gmean = df_algorithm['G-Mean'].values[0]
        ...
        if window_lengths:
            ...
            plt.plot(window_lengths, gmean_values, marker=marker_style, label=legend_name)
    ...
    # Save the plot
    save_dir = os.path.join('plot_G-Means2', f'abtype{abtype}')
    os.makedirs(save_dir, exist_ok=True)
    plot_filename = os.path.join(save_dir, f'abtype{abtype}_t{t_value}_G-Mean_vs_Window_Length.png')
    plt.savefig(plot_filename, bbox_inches='tight')
    plt.close()

def plot_all_gmean_vs_window_length():
    """Plot G-Mean vs Window Length for all abtypes and t values."""
    result_dir = 'results'
    ...
    for abtype in range(1, 8):  # Loop over abtypes from 1 to 7
        ...
        for t_value in t_values:
            plot_gmean_vs_window_length(abtype, t_value)

if __name__ == "__main__":
    plot_all_gmean_vs_window_length()
