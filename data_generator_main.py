    ...
    def generate_binary_class_data(self, abtype):
        ...
        for i in range(self.b):
            ...
            #     data2[i, shift_point:] = np.random.randn(self.w - shift_point) * 1 + 0 + self.t  # After shift
                ...
                shift_point = self.w // 2
                data2[i] = np.random.randn(self.w) * 1 + 0
                data2[i, shift_point:] += self.t  # Add shift magnitude to second half
            elif abtype == 4:  # Downshift
                shift_point = self.w // 2
                data2[i] = np.random.randn(self.w) * 1 + 0
                data2[i, shift_point:] -= self.t  # Subtract shift magnitude from second half
